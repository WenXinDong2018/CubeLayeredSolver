...
ROLL_OUT_DATA = "results/cube3_sequential_500/results.pkl"
...
def getOptions(option_name):
    if option_name == "layer2":
        ...
        return get_roll_out(3, 10, ROLL_OUT_DATA)
    else:
...
def main():
    ...
    parser.add_argument('--env', type=str, required=True, help="Environment: cube3, cube3_layer1, cube3_layer2")
    ...
    parser.add_argument('--model_name', type = str, help="Which model to use when doing search: options in pytorch_models.py")
...
def bwas_python(args, env: Environment, states: List[State], nnet_type: str = "baseline"):
    ...
    heuristic_fn = nnet_utils.load_heuristic_fn(args.model_dir, device, on_gpu, env.get_nnet_model(nnet_type = nnet_type, model_name = args["model_name"]),
...
def bwas_cpp(args, env: Environment, states: List[State], results_file: str):
    assert (args.env.upper() in ['CUBE3', 'CUBE3_LAYER1','CUBE3_LAYER2', 'CUBE4', 'PUZZLE15', 'PUZZLE24', 'PUZZLE35', 'PUZZLE48', 'LIGHTSOUT7'])
    ...
    # Get state dimension
    if args.env.upper() == "CUBE3" or args.env.upper() == "CUBE3_LAYER1" or args.env.upper() == "CUBE3_LAYER2" :
        state_dim: int = 54
    else:
