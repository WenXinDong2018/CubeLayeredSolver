...
def get_heuristic_fn_comp(layer: int, nnet: nn.Module, device: torch.device, env: List[Environment], clip_zero: bool = False,
    ...
    def heuristic_fn(states: List, is_nnet_format: bool = False) -> np.ndarray:
        ...
        while start_idx < num_states:
            ...
            cost_to_go_batch = cost_to_go_batch.reshape((len(states_batch), 3))
...
def heuristic_fn_queue(heuristic_fn_input_queue, heuristic_fn_output_queue, proc_id, env: Environment):
    def heuristic_fn(states):
        states_nnet = env[0].state_to_nnet_input(states)
...
def heuristic_fn_runner(heuristic_fn_input_queue: Queue, heuristic_fn_output_queues, nnet_dir: str,
    ...
    heuristic_fn = None
    if not all_zeros:
        heuristic_fn = load_heuristic_fn(nnet_dir, device, on_gpu, env[0].get_nnet_model( model_name = model_name), env, gpu_num=gpu_num,
